...
fishnet <- 
  st_make_grid(chicagoBoundary,
               cellsize = 500, #specify cell size, this is changeable
               square = TRUE) %>%
  .[chicagoBoundary] %>%            # fast way to select intersecting polygons
  st_sf() %>%
  mutate(uniqueID = 1:n()) #create unique id for each grid, which is important for doing tabular join to put them together
...
final_net.localMorans <- 
  cbind(local_morans, as.data.frame(final_net)) %>% #asking each cell if theyre part of a cluster of abandoned car reports
  st_sf() %>%
...
#local morans i means this cell has clustering vs global morans I -- this is the 'secret sauce'
